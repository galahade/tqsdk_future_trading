...
class Trade_status:
    ...
    def __init__(self, api, position, quote):
        ...
        # 做多交易属性
        self.l_open_price = 0.0
        self.l_open_vols = 0
        self.l_stop_loss_price = 0.0
        self.l_begin_profit = False
        self.l_daily_cond = 0
        ...
        # 做空交易属性
        self.s_open_price = 0.0
        self.s_open_vols = 0
        self.s_stop_loss_price = 0.0
        self.s_begin_profit = False
        self.s_daily_cond = 0
    ...
    def make_long_deal(self):
        ...
        self.l_open_price = self.__position.open_price_long
        ...
        self.l_stop_loss_price = round(self.l_open_price
                                          * (1 - l_base_persent), 2)
        self.l_stop_profit_point = round(self.l_open_price
                                            * (1 + l_base_persent * 3), 2)
    ...
    def make_short_deal(self):
        ...
        if (self.short_or_long != -1 or not self.s_daily_cond or
           ...
           self.__m30_kline.empty):
            ...
            return False
        self.is_trading = True
        self.s_open_price = self.__position.open_price_short
        self.s_open_vols = self.__position.pos_short
        ema9 = self.__h2_kline.ema9
        ema22 = self.__h2_kline.ema22
        close = self.__h2_kline.close
        macd = self.__h2_kline["MACD.close"]
        self.s_stop_loss_price = (self.s_open_price * (1 + s_base_persent))
        self.s_stop_profit_point = (self.s_open_price
                                    * (1 - s_base_persent * 3))
        logger.info(f'{get_date_str(self.__quote.datetime)}'
                    f'<做空>止损设置为:{self.s_stop_loss_price}')
        logger.info(f'{get_date_str(self.__quote.datetime)}'
                    f'<做空>止赢起始价为:{self.s_stop_profit_point}')
        if self.s_daily_cond in [1, 3]:
            self.s_profit_cond = 1
        elif (self.s_daily_cond == 2 and close > ema9 > ema22 and
              macd > 0):
            self.s_profit_cond = 2
            ...
            if (self.short_or_long == 1 and current_price >=
               self.l_stop_profit_point):
                logger.info(log_str.format(get_date_str(self.__quote.datetime),
                                           ...
                                           self.l_stop_profit_point))
                self.l_begin_profit = True
                return True
            elif (self.short_or_long == -1 and current_price <=
                  self.s_stop_profit_point):
                logger.info(log_str.format(get_date_str(self.__quote.datetime),
                                           '做空', self.__quote.last_price,
                                           self.s_stop_profit_point))
                self.s_begin_profit = True
                return True
        return False
            ...
            if self.__match_dk_cond_short():
                if self.__match_2hk_cond_short():
                    if(self.__match_30mk_cond_short()):
                        if self.__match_5mk_cond_short():
                            if self.position.pos_short == 0:
                                return True
        return False
...
满足5分钟线条件,ema60:{ema60},收盘:{close}, MACD:{macd},diff:{diff}")
    ...
    def __match_5mk_cond_short(self):
        logger = get_logger()
        kline = self.m5_klines.iloc[-2]
        ema60 = kline.ema60
        macd = kline['MACD.close']
...
满足两小时线条件2,ema22:{ema22},ema60:{ema60},收盘:{close},MACD:{macd}")
    ...
    def __match_2hk_cond_short(self):
        logger = get_logger()
        kline = self.h2_klines.iloc[-2]
        dk_cond = self.trade_status.s_daily_cond
        ema22 = kline.ema22
        ema60 = kline.ema60
        macd = kline['MACD.close']
        close = kline.close
...
收盘:{close},diff:{diff}")
    ...
    def __match_dk_cond_short(self):
        ...
        if kline["s_qualified"]:
            ...
            diff2 = diff_two_value(ema9, ema60)
            if ema22 > ema9 > ema60:
    ...
    def __try_stop_profit(self):
        logger = get_logger()
        pos_long = self.position.pos_long
        ...
        if pos_long > 0 and (self.trade_status.l_begin_profit
...
现价:{self.quote.last_price},止损价:{self.trade_status.l_stop_loss_price} \
手数:{position_log}")
